...
const AlbumHeader: React.FC<AlbumHeaderProps> = ({ details, songs }) => {
  ...
  const addToLibrary = () => {
    toast('Album added succssefuly to your library', {position: 'bottom-center', hideProgressBar: true});
  }
    ...
    <div className="album-header-card">
      ...
      <div className="info">
        ...
        <div className="atrist">Album  •  {details.songs?.length} Songs</div>
          <button className="icon-button" onClick={() => setSelectedSong(songs[0])}>
          ...
          <button className="icon-button" onClick={() => setSelectedSong(songs[GetRandomSong()])}>
      ...
      <AlbumInfoModal isOpen={isModalOpen} onClose={() => setModalOpen(false)}></AlbumInfoModal>
    </div>
  );
};
